...
def load_metric_config(config_path: str) -> dict:
    ...
    print(f"Loaded metric config from: {config_path}  "
          f"({len(cfg.get('metrics', {}))} metrics)")
    return cfg
...
def _hardcoded_defaults() -> dict:
    ...
    return {"metrics": {}, "osc_window": 50, "osc_step": 5, "osc_lag": 3,
            "defaults": defaults}
...
def _resolve_baseline(metric_name: str, day_df: pd.DataFrame,
                      global_baseline: dict | None) -> tuple[float, float]:
...
def detect_random_spikes(df: pd.DataFrame, bm: float, bs: float,
                         cfg: dict, metric: str) -> list[tuple]:
    spike_z = _get(cfg, metric, "spike_z")
    z = (df["value"] - bm).abs() / bs
    return [(t, t) for t in df[z > spike_z]["timestamp"]]


def detect_step_jumps(df: pd.DataFrame, bm: float, bs: float,
                      cfg: dict, metric: str) -> list[tuple]:
...
def detect_slow_drift(df: pd.DataFrame, bm: float, bs: float,
                      cfg: dict, metric: str) -> list[tuple]:
    ...
    return [(df["timestamp"].iloc[0], df["timestamp"].iloc[-1])] \
        if abs(slope) > drift_thresh else []


def detect_variance_growth(df: pd.DataFrame, bm: float, bs: float,
                           cfg: dict, metric: str) -> list[tuple]:
...
def detect_trend_acceleration(df: pd.DataFrame, bm: float, bs: float,
                               cfg: dict, metric: str) -> list[tuple]:
...
def detect_baseline_shift(df: pd.DataFrame, bm: float, bs: float,
                          cfg: dict, metric: str) -> list[tuple]:
    ...
    if abs(df["roll_mean"].iloc[-1] - bm) > shift_sigma * bs:
        ...
        start_ts  = crossings["timestamp"].iloc[0] if len(crossings) \
            else df["timestamp"].iloc[-1]
        return [(start_ts, df["timestamp"].iloc[-1])]
    return []


def detect_increasing_outlier_frequency(df: pd.DataFrame, bm: float, bs: float,
                                        cfg: dict, metric: str) -> list[tuple]:
    spike_z    = _get(cfg, metric, "spike_z")
    ...
    return [(df["timestamp"].iloc[mid], df["timestamp"].iloc[-1])] \
        if late > freq_mult * early and late > min_count else []


def detect_oscillation_loss(df: pd.DataFrame, bm: float, bs: float,
                             cfg: dict, metric: str) -> list[tuple]:
...
def detect_periodic_oscillation(df: pd.DataFrame, bm: float, bs: float,
                                 cfg: dict, metric: str) -> list[tuple]:
    ...
    for combo_key in df["new_metric_name"].unique():
        ...
        for _, day_df in df[df["new_metric_name"] == combo_key].groupby("date"):
            ...
            for pattern_name, detector_fn in detectors:
                for start_ts, end_ts in detector_fn(day_df, bm, bs,
                                                    metric_cfg, metric_name):
                    rows.append({
...
def print_summary(out: pd.DataFrame) -> pd.DataFrame:
    ...
    print("\n" + "=" * 60)
    print(f"  TOTAL: {len(out)} pattern window(s) across "
...
if __name__ == "__main__":
    ...
    out = run_pattern_pipeline(df, global_baseline=global_baseline,
